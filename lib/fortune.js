...
const AGENT_AI_GUIDANCE = {
  career: '聚焦职位、权责、项目、平台、贵人、收入与进退时机；给出可验证的现实条件和谈判节点。',
  love: '聚焦关系阶段、双方回应、承诺、边界与沟通时机；必须区分单身、暧昧和已有关系的不同情形。',
  health: '聚焦作息、压力、恢复和就医边界；只做生活节律建议，不诊断疾病，不把五行当作病因。',
  outfit: '聚焦用户明确的日期、场合、身份目标、色彩比例、版型和材质；命理只做个人气质的辅助依据。',
  talisman: '聚焦现有器物的材质、用途、安全性和习惯提醒；不宣称超自然效果，不诱导购买。'
};
...
function clipAiText(value, fallback, maxLength) {
  ...
  if (boundary >= Math.floor(maxLength * 0.55)) return clipped.slice(0, boundary + 1);
...
function sanitizeAiReading(value, fallback) {
  ...
  const report = aiReport ? {
    ...fallback.report,
    methodology: clipAiText(aiReport.methodology, fallback.report.methodology, 500),
    basis: fallback.report.basis,
    verdict: {
      heading: clipAiText(aiReport.verdict?.heading, fallback.report.verdict.heading, 52),
      body: clipAiText(aiReport.verdict?.body, fallback.report.verdict.body, 1400)
    },
    timeline: fallback.report.timeline.map((item, index) => ({
      ...item,
      title: clipAiText(aiReport.timeline?.[index]?.title, item.title, 64),
      body: clipAiText(aiReport.timeline?.[index]?.body, item.body, 1400),
      opportunity: clipAiText(aiReport.timeline?.[index]?.opportunity, item.opportunity, 500),
      risk: clipAiText(aiReport.timeline?.[index]?.risk, item.risk, 500)
    })),
    comparisons: Array.isArray(aiReport.comparisons) && aiReport.comparisons.length >= 2
      ? aiReport.comparisons.slice(0, 3).map((item) => ({
        title: clipAiText(item.title, '方案对照', 52),
        body: clipAiText(item.body, '', 1200)
      }))
    ...
    triggers: Array.isArray(aiReport.triggers) && aiReport.triggers.length >= 3
      ? aiReport.triggers.slice(0, 5).map((item) => ({
        condition: clipAiText(item.condition, '判断条件', 120),
        recommendation: clipAiText(item.recommendation, '', 800)
      }))
      : fallback.report.triggers,
    conclusion: {
      heading: clipAiText(aiReport.conclusion?.heading, fallback.report.conclusion.heading, 52),
      body: clipAiText(aiReport.conclusion?.body, fallback.report.conclusion.body, 1400)
    }
  } : fallback.report;
  return {
    ...fallback,
    title: clipAiText(value.title, fallback.title, 52),
    brief: clipAiText(value.brief, fallback.brief, 420),
    action: clipAiText(value.action, fallback.action, 220),
    analysisPath: Array.isArray(value.analysisPath) && value.analysisPath.length === 3
      ? value.analysisPath.map((item, index) => ({
        label: clipAiText(item.label, fallback.analysisPath[index].label, 18),
        value: clipAiText(item.value, fallback.analysisPath[index].value, 220)
      }))
      : fallback.analysisPath,
    details: Array.isArray(value.details) && value.details.length
      ? value.details.slice(0, 4).map((item) => ({
        heading: clipAiText(item.heading, '详细解读', 36),
        body: clipAiText(item.body, '', 900)
      }))
      : fallback.details,
    report,
    generatedBy: 'ai'
...
async function generateAiReading({ user, agentId, question, history, fallback }) {
  ...
  const system = `你是问道的${agent.role}，只处理${agent.name}。${AGENT_AI_GUIDANCE[agentId]}
排盘规则：沿用给定的确定性四柱、十神、大运与流年，不能改写干支和年份，不能编造用户未提供的经历、职位、领导关系、人格类型或家庭状态，也不能把简化旺衰倾向伪装成完整喜用神定论。
差异化要求：第一句直接回答本次具体问题，报告至少显式引用四个当前档案锚点（例如日主、月令、大运、具体流年、十神或关注标签），并解释它们与问题的因果关系。不得照抄参考报告的整句，不得把不同档案都写成同一结论。对不足以确定的现实条件，必须用“如果…则…”给出分支，不假装已知。
上下文要求：历史消息只属于当前档案与当前 Agent；有相关信息时延续，不相关时忽略。
输出严格 JSON：
{"title":"","brief":"","action":"","analysisPath":[{"label":"","value":""},{"label":"","value":""},{"label":"","value":""}],"details":[{"heading":"","body":""}],"report":{"methodology":"","verdict":{"heading":"","body":""},"timeline":[{"title":"","body":"","opportunity":"","risk":""}],"comparisons":[{"title":"","body":""}],"triggers":[{"condition":"","recommendation":""}],"conclusion":{"heading":"","body":""}}}
brief 120-260 字，先结论后因果；analysisPath 恰好三项；details 为 3-4 项；timeline 必须按给定四个年份依次输出；comparisons 至少两项；triggers 至少三项。详细版达到 1400-2400 个中文字符，每段都必须有本档案或本问题的依据，不用万能话术凑字数。`;
  const context = `当前档案（不得与其他档案混用）：
- 姓名：${user.name}；性别：${user.gender}；出生：${user.birthDate} ${user.birthTime}；出生地：${user.birthplace}
- 当前关注：${user.focus?.length ? user.focus.join('、') : '未设置'}；星座：${chart.zodiac}；生肖：${chart.animal}
- 四柱：${chart.baziText}；日主：${chart.dayMaster}${chart.dayElement}；月令：${chart.monthBranch}${chart.monthElement}
- 天干十神：${chart.tenGods.stems.join('、')}；地支藏干十神：${chart.tenGods.branches.map((items) => items.join('/')).join('、')}
- 旺衰倾向：${chart.strength.label}；依据：${chart.strength.explanation}；边界：${chart.strength.disclaimer}
- 平衡侧重：${chart.strength.balancingElements.join('、')}
- 当前大运：${chart.currentLuck ? `${chart.currentLuck.pillar}（${chart.currentLuck.startYear}-${chart.currentLuck.endYear}），运干${chart.currentLuck.stemTenGod}` : '未判定'}
- 未来四年（不得修改顺序与干支）：${chart.yearlyOutlook.map((item) => `${item.year}${item.pillar}，${item.stemTenGod}，${item.tone.label}`).join('；')}
- 今日干支：${chart.today.pillar}${chart.today.element}

本次原问题：${question}

规则引擎参考报告（只用于核对排盘与年份，不得照抄句子）：${JSON.stringify(fallback.report)}`;
  ...
  try {
    const response = await fetch(apiUrl, {
      ...
      signal: AbortSignal.timeout(Number(process.env.AI_TIMEOUT_MS) || 55000)
    });
    if (!response.ok) {
      console.warn('AI reading fallback: provider status', response.status, response.statusText);
      return fallback;
    }
    const payload = await response.json();
    const content = payload.choices?.[0]?.message?.content;
    if (!content) {
      console.warn('AI reading fallback: provider returned no message content');
      return fallback;
    }
    const normalized = typeof content === 'string'
      ? content.trim().replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/, '')
      : content;
    return sanitizeAiReading(typeof normalized === 'string' ? JSON.parse(normalized) : normalized, fallback);
  } catch (error) {
    console.warn('AI reading fallback:', error?.name || 'Error', error?.message || 'Unknown error');
    return fallback;
  }
}

module.exports = { AGENTS, calculateProfile, buildLocalReading, buildOverviewReadings, buildDailyReading, generateAiReading, clipAiText };
